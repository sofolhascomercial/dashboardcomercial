...
function compareMonthKeys(monthKeyA, monthKeyB) {
  const a = String(monthKeyA || '').trim();
  const b = String(monthKeyB || '').trim();
  if (!a && !b) return 0;
  if (!a) return -1;
  if (!b) return 1;
...
function comparePeriodOrder(monthKeyA, weekLabelA, monthKeyB, weekLabelB) {
  const monthDiff = compareMonthKeys(monthKeyA, monthKeyB);
  if (monthDiff !== 0) return monthDiff;
  return weekSortValue(weekLabelA) - weekSortValue(weekLabelB);
}

function compareRecordRecency(recordA, recordB) {
  const periodDiff = comparePeriodOrder(
    recordA?.monthKey || inferRecordMonthKey(recordA),
    recordA?.semana,
    recordB?.monthKey || inferRecordMonthKey(recordB),
    recordB?.semana
  );
  if (periodDiff !== 0) return periodDiff;
  const timeA = new Date(recordA?.dataImportacao || recordA?.importedAt || 0).getTime();
  const timeB = new Date(recordB?.dataImportacao || recordB?.importedAt || 0).getTime();
  return timeA - timeB;
}

function getLatestCostaRecordMap(records) {
  const map = new Map();
  (records || []).forEach(item => {
    if (item?.rede !== 'COSTA' || !item?.loja) return;
    const key = item.loja;
    const current = map.get(key);
    if (!current || compareRecordRecency(current, item) < 0) {
      map.set(key, item);
    }
  });
  return map;
...
function getCostaCurrentStockTotal(records) {
  return [...getLatestCostaRecordMap(records).values()]
    .reduce((sum, item) => sum + Number(item.valorEstoque || 0), 0);
}

function getPreviousCostaStock(store, monthKey, weekLabel, sourceRecords = appState.data) {
  const normalizedStore = normalizeStoreAndNetwork(store, 'COSTA').loja || String(store || '').trim();
  const currentMonthKey = String(monthKey || '').trim();

  const previousRecords = (sourceRecords || []).filter(item => {
    if (item?.rede !== 'COSTA') return false;
    const normalizedItemStore = normalizeStoreAndNetwork(item.loja, 'COSTA').loja || String(item.loja || '').trim();
    if (normalizedItemStore !== normalizedStore) return false;

    const itemMonthKey = String(item.monthKey || inferRecordMonthKey(item) || '').trim();

    // Para a COSTA, a 1ª semana de um mês já vem com o estoque do mês anterior embutido
    // no valor da tabela. Então só devemos descontar o estoque da semana anterior
    // quando ela pertencer ao mesmo mês da semana atual.
    if (!currentMonthKey || itemMonthKey !== currentMonthKey) return false;

    return comparePeriodOrder(
      itemMonthKey,
      item.semana,
      currentMonthKey,
      weekLabel
  ...
  if (!previousRecords.length) return 0;
  previousRecords.sort((a, b) => compareRecordRecency(a, b));
  return Number(previousRecords[previousRecords.length - 1]?.valorEstoque || 0);
}
...
function normalizeStoredRecord(record) {
  ...
  const vendaBruta = Number(record.valorVendaBruta || record.valorVenda || 0);
  const estoqueAnteriorAplicado = Number(record.valorEstoqueAnteriorAplicado || 0);
  ...
  return {
    ...
    valorVenda: venda,
    valorVendaBruta: vendaBruta,
    valorEstoqueAnteriorAplicado: estoqueAnteriorAplicado,
...
function parseImportedWorkbook(workbook, fileName, periodInfo = {}) {
  ...
  workbook.SheetNames.forEach(sheetName => {
    ...
    const parsedSheet = parseDataWorksheet({
      ...
      monthKey,
      periodLabel,
      historicalRecords: appState.data
    });
...
function parseDataWorksheet({ sheetName, rows, batchId, fileName, importedAt, weekLabel, monthLabel, monthKey, periodLabel, historicalRecords = appState.data }) {
  const normalizedSheet = normalizeText(sheetName);
  ...
  const records = dataRows.map((row, index) => {
    ...
    const estoqueAnteriorAplicado = rede === 'COSTA'
      ? getPreviousCostaStock(store, monthKey, weekLabel, historicalRecords)
      : 0;
    const vendaReal = rede === 'COSTA'
      ? Math.max(0, venda - estoqueAnteriorAplicado)
      : venda;
    const quebraTotalCalculada = quebraOperacional + falta + qualidade;

    if ((!percentualQuebra || !Number.isFinite(percentualQuebra)) && vendaReal > 0 && quebraTotalCalculada !== 0) {
      percentualQuebra = Number(((quebraTotalCalculada / vendaReal) * 100).toFixed(2));
    }
    ...
    return {
      ...
      monthKey,
      periodLabel,
      valorVenda: Number(vendaReal.toFixed(2)),
      valorVendaBruta: Number(venda.toFixed(2)),
      valorEstoqueAnteriorAplicado: Number(estoqueAnteriorAplicado.toFixed(2)),
      ...
      percentualFalta: vendaReal > 0 ? Number(((falta / vendaReal) * 100).toFixed(2)) : 0,
      percentualQualidade: vendaReal > 0 ? Number(((qualidade / vendaReal) * 100).toFixed(2)) : 0,
      statusQuebra: getBreakStatus(percentualQuebra).label,
...
function buildSheetVerification(sheetName, records, totalRow) {
  ...
  const totals = aggregateRecords(records);
  const totalEstoque = getCostaCurrentStockTotal(records);
  const estoqueAnteriorAplicadoTotal = records.reduce((sum, item) => sum + Number(item.valorEstoqueAnteriorAplicado || 0), 0);

  const vendaEsperadaBruta = parseMoney(getFlexibleCell(totalRow, [['VALOR', 'ENTREGA', 'TOTAL'], ['VENDA']]));
  const vendaEsperada = model === 'COSTA'
    ? Math.max(0, vendaEsperadaBruta - estoqueAnteriorAplicadoTotal)
    : vendaEsperadaBruta;
  ...
  if (model === 'DIA_A_DIA') {
    ...
    details.push(buildVerificationDetail('Estoque atual', parseMoney(getFlexibleCell(totalRow, [['ESTOQUE', 'EM', 'LOJA'], ['VALOR', 'EST'], ['ESTOQUE']])), totalEstoque, { alwaysInclude: true }));
...
function compareWorkbookTotals(grouped, totalSheet) {
  ...
  return totalSheet.map(summary => {
    ...
    const vendaEsperada = network === 'COSTA'
      ? Math.max(0, Number(summary.venda || 0) - Number(found.estoqueAnteriorAplicado || 0))
      : Number(summary.venda || 0);

    const details = [
      buildVerificationDetail('Venda', vendaEsperada, Number(found.venda || 0), { alwaysInclude: true })
    ];
  ...
  records.forEach(item => {
    ...
    if (!map.has(key)) {
      map.set(key, {
        ...
        valorVenda: 0,
        valorVendaBruta: 0,
        valorEstoqueAnteriorAplicado: 0,
        valorQuebra: 0,
        valorQuebraOperacional: 0,
        valorFalta: 0,
        valorQualidade: 0,
        valorEstoque: 0,
        latestStockRecord: null
      });
    }
    const row = map.get(key);
    row.valorVenda += Number(item.valorVenda || 0);
    row.valorVendaBruta += Number(item.valorVendaBruta || item.valorVenda || 0);
    row.valorEstoqueAnteriorAplicado += Number(item.valorEstoqueAnteriorAplicado || 0);
    ...
    if (item.rede === 'COSTA') {
      if (!row.latestStockRecord || compareRecordRecency(row.latestStockRecord, item) < 0) {
        row.latestStockRecord = item;
...
function renderTopInfo(filtered) {
  ...
  const totalStock = getCostaCurrentStockTotal(displayRecords);
...
function aggregateByNetwork(records) {
  ...
  records.forEach(item => {
    if (!map.has(item.rede)) {
      map.set(item.rede, {
        ...
        venda: 0,
        vendaBruta: 0,
        estoqueAnteriorAplicado: 0,
        quebra: 0,
        quebraOperacional: 0,
        falta: 0,
        qualidade: 0,
        estoque: 0,
        latestCostaStocks: new Map()
      });
    }
    const row = map.get(item.rede);
    row.venda += Number(item.valorVenda || 0);
    row.vendaBruta += Number(item.valorVendaBruta || item.valorVenda || 0);
    row.estoqueAnteriorAplicado += Number(item.valorEstoqueAnteriorAplicado || 0);
    ...
    if (item.rede === 'COSTA') {
      const current = row.latestCostaStocks.get(item.loja);
      if (!current || compareRecordRecency(current, item) < 0) {
        row.latestCostaStocks.set(item.loja, item);
      }
    } else {
  ...
  return [...map.values()].map(item => {
    const estoqueCosta = [...item.latestCostaStocks.values()]
      .reduce((sum, record) => sum + Number(record.valorEstoque || 0), 0);
    return {
      ...item,
      estoque: Number((item.estoque + estoqueCosta).toFixed(2)),
      latestCostaStocks: undefined,
