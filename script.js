...
function buildMetaInputs() {
  ...
  setCurrencyInputValue(els.metaGeralInput, appState.config.metaGeral || 0);
  ...
  attachCurrencyMask(els.metaGeralInput);
  [...els.metasRedeForm.querySelectorAll('input[data-network]')].forEach(attachCurrencyMask);
}
...
function saveGoals() {
  appState.config.metaGeral = parseCurrencyInput(els.metaGeralInput.value);
